...
geneInfo2 <- geneInfo[,c(1,4)] %>% unique()

AllORFs <- left_join(AllORFs, geneInfo2)
...
#ggsave(plot = nuORF_specific, filename = here("plots", "nuORFs_Betaspecific.pdf"), width = 4, height = 3)
```
...
TYKvars <- read_csv(here("accessories", "kp_variant_TYK_regions.csv"))

TYKvars$TYK <- "TYK2"

colnames(TYKvars)

ggviolin(TYKvars, y="Beta__T1D", x="TYK", trim = F, add=c("mean"))+ theme_minimal() + ylab("log10(Odds Ratio)") + xlab("") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

TYK_regions4 <- read_tsv(here("accessories", "TYK_regions4.tsv"), col_names = FALSE)
TYK_regions4 <- TYK_regions4 %>% arrange(desc(X2))

TYK_regions4 %>%
 ggplot( aes(x=X2, y=X3)) +
    geom_point() + theme_minimal() + ylab("Log10(Odds Ratio)") + xlab("Position - 5'-> 3'")
