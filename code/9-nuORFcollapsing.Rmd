...
AllORFs <- read_xlsx(here::here("output", "AllORFs_wProteomics_filtered.xlsx"))
AllORFs <- AllORFs[,1:6]
...
proteomicsRC <- read_tsv(here::here("proteomics", "RiboCode", "sBC_nORF", "combined_protein.tsv")) %>% filter(`Combined Unique Spectral Count` > 1) %>% as.data.frame()
...
proteomicsOQ <- read_tsv(here::here("proteomics", "ORFQuant", "sBC_nORF", "combined_protein.tsv")) %>% filter(`Combined Unique Spectral Count` > 1) %>% as.data.frame()
...
#ggsave(plot = p_nuORFs_percent, filename = here("plots", "nuORFs_percet.pdf"), width = 4, height = 3)
...
#ggsave(plot = p_nuORFoverlap, filename = here("plots", "nuORFoverlap_venndiagram.pdf"), width = 4, height = 3)
...
tableProteomicSupport <- tableProteomicSupport %>% filter(ORF_type != "Top20TE" & program != "None")

tableProteomicSupport$ORF_type <- factor(tableProteomicSupport$ORF_type, order = TRUE, levels =c('annotated', 'novel', 'uORF', "dORF", "other", "Top20TE"))
levels(tableProteomicSupport$ORF_type)

p_nuORF_proteoSupport_percent <- ggplot(tableProteomicSupport %>% filter(ORF_type != "annotated" & ORF_type != "other" & ORF_type != "Top20TE"), aes(fill=protein, y=Freq, x=program)) + 
    geom_bar(position="fill", stat="identity") + theme_minimal() + scale_fill_colorblind() + ylab("Percent") + xlab("") +
  facet_grid(~ ORF_type) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

p_nuORF_proteoSupport_percent

#ggsave(plot = p_nuORF_proteoSupport_percent, filename = here("plots", "nuORFProteinSupport_percent.pdf"), width = 5, height = 3)

tableProteomicSupport_collapsed <- AllORFs %>% group_by(ORF_type) %>% select(protein) %>% table() %>% as.data.frame()

tableProteomicSupport_collapsed <- tableProteomicSupport_collapsed %>% filter(ORF_type != "Top20TE")

tableProteomicSupport_collapsed$ORF_type <- factor(tableProteomicSupport_collapsed$ORF_type, order = TRUE, levels =c('novel', 'uORF', "dORF", "other", "Top20TE"))
levels(tableProteomicSupport_collapsed$ORF_type)

p_nuORF_proteoSupport_percent_collapsed <- ggplot(tableProteomicSupport_collapsed %>% filter(ORF_type != "annotated" & ORF_type != "other" & ORF_type != "Top20TE"), aes(fill=protein, y=Freq, x=ORF_type)) + 
...
#ggsave(plot = p_nuORF_proteoSupport_percent_collapsed, filename = here("plots", "nuORFProteinSupport_percent_collapsed.pdf"), width = 5, height = 3)

p_nuORF_proteoSupport_count <- ggplot(tableProteomicSupport %>% filter(ORF_type != "annotated" & ORF_type != "other" & ORF_type != "Top20TE"), aes(fill=protein, y=Freq, x=program)) + 
    geom_bar(position="stack", stat="identity") + theme_minimal() + scale_fill_colorblind() + ylab("Count") + xlab("") +
  facet_grid(~ ORF_type) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

p_nuORF_proteoSupport_count

#ggsave(plot = p_nuORF_proteoSupport_count, filename = here("plots", "nuORFProteinSupport_counts.pdf"), width = 5, height = 3)
```
...
AllORFs$ORF_type <- factor(AllORFs$ORF_type, order = TRUE, levels =c('annotated', 'novel', 'uORF', "dORF", "other", "Top20TE"))
...
p_nuORF_lengthAA_anno <- ggviolin(AllORFs %>% filter(ORF_type != "other" & ORF_type != "Top20TE"), y="nchar", x="ORF_type", fill = "ORF_type", trim = F, add=c("mean"))+ theme_minimal() + scale_fill_manual(values = colors) + ylab("ORF Length - AA") + xlab("") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + scale_y_continuous(trans="log10",
    breaks = trans_breaks("log10", function(x) 10^x),
    labels = trans_format("log10", math_format(10^.x))) +
  annotation_logticks(sides = "l") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
#ggsave(plot = p_nuORF_lengthAA_anno, filename = here("plots", "nuORF_length_anno.pdf"), width = 5, height = 4)

#ggsave(plot = p_nuORF_lengthAA_anno2, filename = here("plots", "nuORF_length_anno2.pdf"), width = 5, height = 4)
...
#ggsave(plot = geneTypeNovel, filename = here("plots", "nuORF_geneTypeNovel.pdf"), width = 5, height = 4)
```




```{r gene expression, message=FALSE, warning=FALSE, echo=FALSE}
#didn't actually use this. No reason to. 
#first I'm just asking for all the nuORFs called by each software and that have proteomics support, do they have similar geneIDs? Is there overlap? 
geneIDs_RC <- AllORFs %>% filter(program == "Ribocode" & ORF_type != "annotated" & protein == "Yes") %>% pull(gene_id, ORF_type) %>% unique()
geneIDs_OQ <- AllORFs %>% filter(program == "ORFquant"  & ORF_type != "annotated" & protein == "Yes") %>% pull(gene_id, ORF_type) %>% unique()


set1 <- as.matrix(geneIDs_RC)[,1]
set3 <- as.matrix(geneIDs_OQ)[,1]

listALL <- list(RiboCode=set1, ORFquant = set3)

#ggVennDiagram(listALL, edge_color = "black", label = "count", label_alpha = 0) + theme(legend.position = "none") #overall about half of each software's called nuORFs are shared from the same geneID. 


#Now I want to get the expression of each gene and see if some categories are coming from genes with high or low expression. Going to use the GFP+ RNA-seq average of replicates for this number. 
...
p_violinNUORFrnaExpress_program <- ggviolin(data = AllORFs_violin %>% filter(ORF_type != "other" & ORF_type != "Top20TE"), x = "program", y = "GFP_avg", trim = F, add=c("mean"), fill = "ORF_type") +
  ylab("log10(rnaTPM)") +
  xlab("") +
...
p_violinNUORFrnaExpress <- ggviolin(data = AllORFs_violin2 %>% filter(ORF_type != "other" & ORF_type != "Top20TE"), x = "ORF_type", y = "GFP_avg",
...
#ggsave(plot = p_violinNUORFrnaExpress, filename = here("plots", "nuORF_rnaExpress_violin.pdf"), width = 5, height = 3)
...
AllORFs_violin_ribo$ORF_type <- factor(AllORFs_violin_ribo$ORF_type, order = TRUE, levels =c("Top20TE", 'annotated', 'novel', 'uORF', "dORF", "other"))

colors <- c("#969696", "#009E73", "#56B4E9", "#E69F00", "#CC79A7", "#551A8B")


p_violinNUORF_TE <- ggviolin(data = AllORFs_violin_ribo %>% filter(ORF_type != "other" & ORF_type != "Top20TE"), x = "ORF_type", y = "TE", fill = "ORF_type",
...
#ggsave(plot = p_violinNUORF_TE, filename = here::here("plots", "nuORF_TE_violin.pdf"), width = 5, height = 3)
...
#ggsave(plot = p_violinNUORF_TE_program, filename = here("plots", "nuORF_TE_violin_program.pdf"), width = 7, height = 4)
...
AllORFs2$ORF_type <- factor(AllORFs2$ORF_type, order = TRUE, levels =c("Top20TE", 'annotated', 'novel', 'uORF', "dORF", "other"))
...
#ggsave(plot = p_nuORF_proteomicIntensity, filename = here("plots", "nuORF_proteomicIntensity.pdf"), width = 6, height = 3)
...
```{r}
#distribution of expression levels by category in RNA-seq
#kozak sequence
  #criteria from: Hernández G., Osnaya V.G. and Pérez-Martínez X. (2019) Conservation and variability of the AUG initiation codon context in eukaryotes. Trends Biochem. Sci., 44, 1009–1021.
#evolutionary score
  #PhyloCSF score
  #PhastCons score
  #GERP score
  #MOEUF score
#GSEA on each category?
#number of nuORFs per gene - related to anything? maybe expression? especially with uORFs.
