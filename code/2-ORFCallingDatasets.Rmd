...
#In this file I am filtering the ORF calls from Ribocode and ORFquant programs. These programs were ran previously on the cluster. The data here is used as a precursor data in Figures 2,3, and 4 and associated Supplemental Figures.  
...
#2. Filtering ORFs based on expression
  #Filtering Ribocode
...
# 2. Filtering ORFs based on expression #START HERE - FIGURE OUT REPRODUCIBLE GFFCOMPARE


```{r Filtering Ribocode, warning=FALSE, message=FALSE}

#Keep only ORFs from expressed transcripts. 
ribocodeTPMfiltered <- ribocode %>% dplyr::filter(transcript_id %in% tpms$ensembl_gene_id)

#Make sure novel categories aren't just alternative isoforms or completely contained in CDS of annotated. 
...
ribocodeTPMfiltered <- ribocodeTPMfiltered %>% filter(keepColumn == "keep")


```


# Ribocode AA collapsing
...
```{r AA collapsing ribocode, message=FALSE, warning=FALSE, echo=FALSE}
...
#write_csv(riboCodeAAFiltered, here("ORFcalling", "Expression Filtered ORFcalling", "RiboCodeFiltered.csv"))

#only the ORF_id and sequence to make a fasta with. 
#write_csv(riboCodeAAFiltered[,c(1,2)], here("ORFcalling", "Fasta Files for Alignment", "RibocodeFilteredFasta.csv"))
```
...
orfQuant_TMPfiltered <- orfquant_minimal %>% filter(transcript_id %in% tpms$ensembl_gene_id)
...
#need to fix annotated category to be all the same.
...
orfQuant_TMPfiltered <- orfQuant_TMPfiltered %>% filter(keepColumn == "keep")
...
#first I want to collapse ORFquant down to the gene level. 
...
write_csv(ORFquantAAFiltered, here("ORFcalling", "Expression Filtered ORFcalling", "ORFquant_Filtered.csv"))

#only the ORF_id and sequence to make a fasta with. 
write_csv(ORFquantAAFiltered[,c(1,2)], here("ORFcalling", "Fasta Files for Alignment", "ORFquantFilteredFasta.csv"))
```
